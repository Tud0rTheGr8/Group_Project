...
```{r tidy retired_time, message=FALSE, warning=FALSE}
...
```{r create object for time calculation, message=FALSE, warning=FALSE}
...
while (i <= nrow(formula_e_time)){
  if (bufferdate != formula_e_time[i,1]){
    ...
    formula_e_time[i,10] <- 3600*buffer_hour + 60*buffer_minute + buffer_second
  }else{
    formula_e_time[i,10] <- 3600*(buffer_hour + formula_e_time[i,7]) + 60*(buffer_minute + formula_e_time[i,8]) + buffer_second + formula_e_time[i,9]
  }
  i <- i+1
}
```

```{r joining result to formula_e_tidy, message=FALSE, warning=FALSE}
formula_e_time <- select(formula_e_time, id, total_second)
formula_e_tidy = inner_join(formula_e_tidy, formula_e_time, by = "id")
```

```{r calculate average rank, message=FALSE, warning=FALSE}
formu <- formula_e %>% 
...
```
```{r rank 1 times over time, message=FALSE, warning=FALSE}
...
```{r top 5 frequency, message=FALSE, warning=FALSE}
